...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    const data = fs.readFileSync('./storage/yep_desc_terms.csv')
    .toString() // convert Buffer to string
    .split('\n') // split string to lines
    .map(e => e.trim()) // remove white spaces for each line
    .map(e => e.split(',').map(e => e.trim())); // split each line to array

  return queryInterface.bulkInsert('yep_desc_terms', data.map(el => {
    let tab = [];
    let ch = "";
    let p = false;
    ...
    return {
      ...
      new_ev: tab[16]== '' ? null : tab[16],
      new_heading_class: tab[17]== '' ? null : tab[17],
      replace_flag_yn: tab[18]== '' ? null : tab[18],
      replace_ui: tab[19]== '' ? null : tab[19],
      replace_term_ui: tab[20]== '' ? null : tab[20],
      replace_term_name: tab[21]== '' ? null : tab[21],
      dcms_pt_ui: tab[22]== '' ? null : tab[22],
      change_type: tab[23]== '' ? null : tab[23],
      createdAt: new Date(),
      updatedAt: new Date()
    }
  }));
  ...
  down: async (queryInterface, Sequelize) => {
    return queryInterface.bulkDelete('yep_desc_terms', null, {});
  }
};
