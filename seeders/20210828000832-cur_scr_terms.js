...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    const data = fs.readFileSync('./storage/cur_scr_terms.csv')
    .toString() // convert Buffer to string
    .split('\n') // split string to lines
    .map(e => e.trim()) // remove white spaces for each line
    .map(e => e.split(',').map(e => e.trim())); // split each line to array

  return queryInterface.bulkInsert('cur_scr_terms', data.map(el => {
    let tab = [];
    let ch = "";
    let p = false;
    ...
    return {
      ...
      relocation_type: tab[14] == '' ? null : tab[14],
  ...
  down: async (queryInterface, Sequelize) => {
    return queryInterface.bulkDelete('cur_scr_terms', null, {});
  }
};
