...
module.exports = {
  up: async (queryInterface, Sequelize) => {
    const data = fs.readFileSync('./storage/yep_desc_pref_terms.csv')
    .toString() // convert Buffer to string
    .split('\n') // split string to lines
    .map(e => e.trim()) // remove white spaces for each line
    .map(e => e.split(',').map(e => e.trim())); // split each line to array

  return queryInterface.bulkInsert('yep_desc_pref_terms', data.map(el => {
    let tab = [];
    let ch = "";
    let p = false;
    ...
    return {
      ...
      old_value: tab[5]== '' ? null : tab[5],
      old_term_type: tab[6]== '' ? null : tab[6],
      old_term_ui: tab[7] == '' ? null : tab[7],
      old_term_name: tab[8] == '' ? null : tab[8],
  ...
  down: async (queryInterface, Sequelize) => {
    return queryInterface.bulkDelete('yep_desc_pref_terms', null, {});
  }
};
